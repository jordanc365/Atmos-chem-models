# -*- coding: utf-8 -*-
"""
Create spatial representation of temporal variability CTM 2013
...
summert = [sum(x) for x in zip(dectav, jantav, febtav)]
summertav = np.array(summert)/3
summertres = summertav.reshape(3600)

summerb = [sum(x) for x in zip(decav, janav, febav)]
summerbav = np.array(summerb)/3
summerbres = summerbav.reshape(3600)

##########################################

autumnt = [sum(x) for x in zip(martav, aprtav, maytav)]
autumntav = np.array(autumnt)/3
autumntres = autumntav.reshape(3600)

autumnb = [sum(x) for x in zip(marav, aprav, mayav)]
autumnbav = np.array(autumnb)/3
autumnbres = autumnbav.reshape(3600)

##########################################

wintert = [sum(x) for x in zip(juntav, jultav, augtav)]
wintertav = np.array(wintert)/3
wintertres = wintertav.reshape(3600)

winterb = [sum(x) for x in zip(junav, julav, augav)]
winterbav = np.array(winterb)/3
winterbres = winterbav.reshape(3600)

##########################################

springt = [sum(x) for x in zip(septav, octtav, novtav)]
springtav = np.array(springt)/3
springtavres = springtav.reshape(3600)

springb = [sum(x) for x in zip(sepav, octav, novav)]
springbav = np.array(springb)/3
springbres = springbav.reshape(3600)

###############################################################################
# seasonal averages 1 number

decav2 = np.mean(dec, axis=(0, 1, 2, 3))
janav2 = np.mean(jan, axis=(0, 1, 2, 3))
febav2 = np.mean(feb, axis=(0, 1, 2, 3))
summeravb =np.array([decav2, janav2, febav2])

##########################################
marav2 = np.mean(mar, axis=(0, 1, 2, 3))
aprav2 = np.mean(apr, axis=(0, 1, 2, 3))
mayav2 = np.mean(may, axis=(0, 1, 2, 3))
autumnavb =np.array([marav2, aprav2, mayav2])

##########################################

junav2 = np.mean(jun, axis=(0, 1, 2, 3))
julav2 = np.mean(jul, axis=(0, 1, 2, 3))
augav2 = np.mean(aug, axis=(0, 1, 2, 3))
winteravb =np.array([junav2, julav2, augav2])

##########################################

sepav2 = np.mean(sep, axis=(0, 1, 2, 3))
octav2 = np.mean(oct, axis=(0, 1, 2, 3))
novav2 = np.mean(nov, axis=(0, 1, 2, 3))
springavb =np.array([sepav2, octav2, novav2])

##############################################################################
# Correlation using for loops temp and bio
dat = np.zeros((summertav.shape[0], summertav.shape[1]))
for ii in range(summertav.shape[0]):
    for jj in range(summertav.shape[1]):
        vtmp = vflip[ii, jj]#.mean(axis=(0, 1))
        ttmp = febav[ii, jj]#.mean(axis=(0, 1))
        dat[ii, jj] = np.correlate([vtmp], [ttmp])/55.1025751877
...
labels = ['North', '', '  ', '   ']
for label, xpt, ypt in zip(labels, x, y):
    plt.text(xpt-20000, ypt+30000, label, color ='black')
fontsize = 40
'''
...
cb.set_label('Average Ambient Temperature ($^o$c)', fontsize=15)

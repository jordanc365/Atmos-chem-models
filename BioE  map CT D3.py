# -*- coding: utf-8 -*-
"""
Create pcolourmesh CTM 2011 Domain 3
...
# [ time, source, lon, lat ]
# plot Daily average
v = f.variables['store_Bio'][:, :, 0, :, :]
v2 = f.variables['store_Bio'][:, 0:24, 0, 06:46, 6:48]

v2av = v2.mean(axis=(0, 1))
v2res= v2av.reshape(1680)/9
tres = t1.reshape(1680)

rcParams['figure.figsize'] = 15, 5
plt.plot(v2res, linestyle='-', linewidth=1.0, c='b',
         label='Biogenic Emissions')
         
plt.plot(tres, linestyle='-', linewidth=1.0, c='b',
         label='Biogenic Emissions')
...
ccres= cc1.reshape(5476)
#vres= v1.reshape(5476)
...
for ii in range(v.shape[2]):
        for jj in range(v.shape[3]):
            vtmp = v[:, :, ii, jj].mean(axis=(0, 1))
            ttmp = cc[:, :, ii, jj].mean(axis=(0, 1))
            dat2[ii, jj] = np.correlate([vtmp], [ttmp])/30.043296814
...
cb.set_label('Average Percentage Cloud Cover', fontsize=15)

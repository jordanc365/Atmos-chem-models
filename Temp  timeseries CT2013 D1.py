...
#temp
yeart = np.concatenate((jantres,febtres,martres,aprtres,maytres,juntres,jultres,augtres,septres,octtres,novtres,dectres))

# std devs 
stdevkoeht1 = np.std(yeart, axis=(1, 2))
stdeight1 = stdevkoeht1[0:8665:12]

tmean = np.average(yeart, axis=(1, 2))

#8 hour avareage 
yeareightt = tmean[0:8665:12]
...
date = np.arange(yeareightt.shape[0])  # assume  delta time between data is 1 hour
date1 = (date/60.)  # use days instead of hours
ar = np.array(date1)
...
plt.plot(ar, yeareightt, linestyle='-', linewidth=0.6, c='r',
         label='Average Ambient Temperature ($^o$c)')

# Create standard devation fill
plt.fill_between(ar,  yeareightt-stdeight1,  yeareightt+stdeight1, alpha=0.2, edgecolor='black', 
                 facecolor='red', linewidth=0.3)
                 
############################################################################################################################################################                                 

plt.plot(ar, yeareight*3, linestyle='-', linewidth=0.6, c='b',
         label='Average Total Biogenic Emissions (kg/$km^2$/hr) (x3)')

# Create standard devation fill
plt.fill_between(ar,  yeareight-stdeight,  yeareight+stdeight*3, alpha=0.2, edgecolor='black', 
...
# tick
plt.xticks(range(0, 13, 1 ), [str(i) for i in range(0, 13, 1)])
plt.yticks(np.arange(-10, 41, 5))  # use floats for y value

# -*- coding: utf-8 -*-
"""
Create speciated emissions Timeseries MEGAN coupled to CTM
...
sr2 = vcon.reshape(648, 41, 42)
smr2 = tercon.reshape(648, 41, 42)
sr3 = tempcon.reshape(648, 41, 42)

#calculate standard devation along lat and lons
stdevkmeg1 = np.std(sr2, axis=(1, 2))/9
stdevkmeg2 = np.std(smr2, axis=(1, 2))/9
...
##############################################################################
plt.plot(date31, sm1, linestyle='-', linewidth=1.2,
         c='k', label=' CSIRO-CTM-MEGAN Monoterpene Emissions')
         
# Create standard devation fill
plt.fill_between(date31, sm1-stdevkmeg2, sm1+stdevkmeg2, alpha=0.5, edgecolor='black', 
                 facecolor='black', linewidth=0.3)

############################################################################################################################################################             

plt.plot(date31, sv1, linestyle='--', linewidth=2.0,
         c='r', label=' CSIRO-CTM-MEGAN Isoprene Emissions')
         
# Create standard devation fill
plt.fill_between(date31, sv1-stdevkmeg1, sv1+stdevkmeg1, alpha=0.3, edgecolor='black', 
...
plt.yticks(np.arange(0, 49, 2))  # use floats for y 

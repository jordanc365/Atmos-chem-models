...
Regrid 2011 emissions to 10x10
...
LAIt = np.array(f.variables['store_Bio'][:, 0:24, 0, :, :])
LAI = LAIt
...
#interpolate
LAInew=scipy.interpolate.griddata((X.flatten(),Y.flatten()),LAI.flatten() , (XI,YI),method='linear')/5-0.05
...
# plot the data
col=map.pcolormesh(glon3,glat3,mocedata,latlon=True,vmin=0, vmax = 1, alpha=trans)

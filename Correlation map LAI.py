...
ft = netCDF4.Dataset('O:\Honours_data\JMEGAN\LAI\laiv200302_30sec.nc','r')

# old 1-km grid
glon1 = ft.variables['lon'][:]
glat1 = ft.variables['lat'][:]



LAI = np.array(ft.variables['LAI_for_Feb_2003_(m2_per_m2)'])*0.001
...
for ii in range(totarrdf.shape[2]):
    for jj in range(totarrdf.shape[3]):
        ...
        ttmp = LAInew[ii, jj] #.mean(axis=(0, 1))
        dat[ii, jj] = np.correlate([vtmp], [ttmp])/43.481079763

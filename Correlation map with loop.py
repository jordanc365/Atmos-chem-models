# -*- coding: utf-8 -*-
"""
Create spatial representaion of temporal variability MEGAN coupled to CTM
...
vcon = np.concatenate((sv2, sv), axis=1)
tercon = np.concatenate((sm2, sm), axis=1)
...
totd = [sum(x) for x in zip(vcon, tercon)]
...
dat = np.zeros((totarrd.shape[2], totarrd.shape[3]))
for ii in range(totarrd.shape[2]):
    for jj in range(totarrd.shape[3]):
        vtmp = totarrd[:, :, ii, jj].mean(axis=(0, 1))
        ttmp = t[:, :, ii, jj].mean(axis=(0, 1))
        dat[ii, jj] = np.correlate([vtmp], [ttmp])/228.728561401
        
# Correlation using for loops temp and bio
dat2 = np.zeros((totarrd.shape[2], totarrd.shape[3]))
for ii in range(totarrd.shape[2]):
    for jj in range(totarrd.shape[3]):
        vtmp = totarrd[:, :, ii, jj].mean(axis=(0, 1))
        ttmp = lait[ii, jj]#.mean(axis=(0, 1))
        dat2[ii, jj] = np.correlate([vtmp], [ttmp])/46.4336547852
...
mocedata = maskoceans(mlons, mlats, dat2, inlands=True, resolution='h',
                      grid=1.25)
...
'''
# Add colour bar
col = map.pcolormesh(mlons, mlats, mocedata, latlon=True, alpha=trans, vmax=max, vmin=min, cmap=cool)
'''
# Add scale bar
